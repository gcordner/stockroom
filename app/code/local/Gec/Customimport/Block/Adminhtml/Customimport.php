...
{ 
    ...
    public function importAllProducts($products,$Currfilepath=null,$Errfilepath=null){
        ...
        foreach($products as $product){
			...
			$this->customHelper->writeCustomLog("xmlimport : Start process for Product # ".$product->id, $this->logPath);
            ...
            // start ktpl0123 custom log
			$this->customHelper->writeCustomLog("xmlimport : End process for Product # ".$product->id, $this->logPath);
			// end ktpl0123 custom log
        }
        Mage::log("xmlimport : Successfully created products: {$this->_created_num}");              
        Mage::log("xmlimport : Successfully updated products: {$this->_updated_num}");
        // start ktpl0123 custom log
		$this->customHelper->writeCustomLog("xmlimport : Successfully created products: {$this->_created_num}", $this->logPath);
		$this->customHelper->writeCustomLog("xmlimport : Successfully updated products: {$this->_updated_num}", $this->logPath);
		// end ktpl0123 custom log
        $this->_created_num = 0;
        $this->_updated_num = 0;
    }
    ...
    public function importAllCategory($categories){
        ...
        foreach($categories  as $category){
            Mage::log("xmlimport : Start process for category # ".$category->id);
            // start ktpl0123 custom log
			$this->customHelper->writeCustomLog("xmlimport : Start process for category # ".$category->id, $this->logPath);
			// end ktpl0123 custom log
            $this->importCategory($category);
            Mage::log("xmlimport : End process for category # ".$category->id);
            // start ktpl0123 custom log
			$this->customHelper->writeCustomLog("xmlimport : End process for category # ".$category->id, $this->logPath);
			// end ktpl0123 custom log
        }
        Mage::log("xmlimport : Successfully created categories: {$this->_created_num}" ); //code for generate log
 	    ...
 	    // start ktpl0123 custom log
		$this->customHelper->writeCustomLog("xmlimport : Successfully created categories: {$this->_created_num}", $this->logPath);
		$this->customHelper->writeCustomLog("xmlimport : Successfully updated categories: {$this->_updated_num}", $this->logPath);
		// end ktpl0123 custom log
 	    $this->_created_num = 0;
        $this->_updated_num = 0;
    }
    ...
    public function parseAllCategoryRelation(){
        ...
        foreach($this->_cat_relation as $catRelation){
            ...
            if($externall){				  //check if parent id exists.
                ...
                Mage::log('xmlimport : category not found: '.$parent);
                // start ktpl0123 custom log
				$this->customHelper->writeCustomLog('xmlimport : category not found: '.$parent, $this->logPath);
				// end ktpl0123 custom log
            }
        }
    }
        ...
        try {
            ...
            if ($validate !== true) {
                foreach ($validate as $code => $error) {
                    if ($error === true) {
						// start ktpl0123 custom log
						$this->customHelper->writeCustomLog(Mage::helper('catalog')->__('xmlimport : Attribute "%s" is required.', $code), $this->logPath);
						$this->customHelper->sendLogEmailAndRemoveLog($this->logPath);
						// end ktpl0123 custom log						
                        Mage::throwException(Mage::helper('catalog')->__('xmlimport : Attribute "%s" is required.', $code));
                    }
                    else {
						// start ktpl0123 custom log
						$this->customHelper->writeCustomLog($error, $this->logPath);
						$this->customHelper->sendLogEmailAndRemoveLog($this->logPath);
						// end ktpl0123 custom log
	    ...
	    foreach($allCats as $category){
	        ...
	        if($subcats != ''){
		        ...
		        if($duplicatedSubcategoryId){		        						
		        	...
		        	// start ktpl0123 custom log
					$this->customHelper->writeCustomLog('xmlimport : error while duplicating Category', $this->logPath);
					// end ktpl0123 custom log
		        }
	        }
	    }
        ...
        if($actualSubId){
            if(count($actualSubId) == 1){
            	...
            	//	echo 'subcat present in this cat '.$ext_subid;
            		...
            		// category is not under any other parent , move           		
                	echo "<br/><br/>".'xmlimport : block will not  execute as  category is not under any other parent'."<br/><br/>";	
                	// start ktpl0123 custom log
					$this->customHelper->writeCustomLog("<br/><br/>".'xmlimport : block will not  execute as  category is not under any other parent'."<br/><br/>", $this->logPath);
					// end ktpl0123 custom log
            	}	
            }                
        }
            ...
            if(count($actualSubId) == 0){
                ...
                // start ktpl0123 custom log
				$this->customHelper->writeCustomLog('xmlimport : subcategory id not found:'.$ext_subid, $this->logPath);
				// end ktpl0123 custom log
            }
        }
    }
    ...
    public function importItem( &$item,$Currfilepath=null,$Errfilepath=null){
        ...
        if(!isset($item->id) || trim($item->id)==''){
            ...
            // start ktpl0123 custom log
			$this->customHelper->writeCustomLog('xmlimport : sku not found for product record #:'.$this->_current_row, $this->logPath);
			// end ktpl0123 custom log
            return false;
        }
        ...
        if(!isset($item->type) || trim($item->type)==''){
            ...
            // start ktpl0123 custom log
			$this->customHelper->writeCustomLog('xmlimport : Product type not found for product record # '.$item->id, $this->logPath);
			// end ktpl0123 custom log
        ...
        if(!isset($pid))
        {
            if(!isset($asid)){
                ...
                // start ktpl0123 custom log
				$this->customHelper->writeCustomLog('xmlimport : cannot create product sku:'.(string)$item->id, $this->logPath);
				// end ktpl0123 custom log
                return false;
            }
            if((string)$item->type == 'configurable'){
                ...
                // start ktpl0123 custom log
				$this->customHelper->writeCustomLog("xmlimport : Import function does not support product type of record: {$item->id}", $this->logPath);
				// end ktpl0123 custom log
            }
    ...
    public function createConfigurableProduct(&$item, $asid){
        ...
        if(count($superattribute_array) > 0){
            ...
            if($super_attribute_created){
                ...
                // start ktpl0123 custom log
				$this->customHelper->writeCustomLog("xmlimport : Could not get super attribute for product. Hence skipped product : {$item->id}", $this->logPath);
				$this->customHelper->writeCustomLog("<br/><br/>".'Could not get super attribute for product. Hence skipped product ' .(string)$item->id ."<br/><br/>", $this->logPath);
				// end ktpl0123 custom log
            }
        }else{
            ...
            // start ktpl0123 custom log
			$this->customHelper->writeCustomLog("xmlimport : Super attribute is missing. Hence skipped product : {$item->id}", $this->logPath);
			$this->customHelper->writeCustomLog("<br/><br/>".'Super attribute is missing. Hence skipped product ' .(string)$item->id ."<br/><br/>", $this->logPath);
			// end ktpl0123 custom log
        }
        }else{
        	...
        	// start ktpl0123 custom log
			$this->customHelper->writeCustomLog("xmlimport : Attribute set ID #$attid is missing. Hence skipped product SKU #$item->id", $this->logPath);
			$this->customHelper->writeCustomLog("<br/><br/>"."Attribute set ID #$attid is missing. Hence skipped product SKU #$item->id" . "<br/><br/>", $this->logPath);
			// end ktpl0123 custom log
        }
    }
    public function createProduct(&$item, $asid){
    	...
    	foreach($multiple_values as $attribute_code=>$attribute_values){
    		...
    		if(!$attr_id){
    			...
    			// start ktpl0123 custom log
				$this->customHelper->writeCustomLog("xmlimport : Skipped product {$item->id}, attribute is not available in magento database.: {$attribute_code}", $this->logPath);
				// end ktpl0123 custom log
    			$skipStatus = 1;
    			break;
    		}
    	...
    	try{
    		if($skipStatus == 0){
    			...
    			if ($productId) {
    				...
    				// start ktpl0123 custom log
					$this->customHelper->writeCustomLog('xmlimport : Skipped product due to improper attribute values :'.(string)$item->id, $this->logPath);
					$this->customHelper->writeCustomLog('Skipped product due to improper attribute values :'.(string)$item->id, $this->logPath);
					// end ktpl0123 custom log
    			}
    		}else{
    			...
    			// start ktpl0123 custom log
				$this->customHelper->writeCustomLog('xmlimport : Skipped product due to some error while save : '.(string)$item->id, $this->logPath);
				$this->customHelper->writeCustomLog('Skipped product due to some error while save : '.(string)$item->id, $this->logPath);
				// end ktpl0123 custom log
    		}
    	}
    		...
    		// start ktpl0123 custom log
			$this->customHelper->writeCustomLog("xmlimport : Attribute set ID #$attid is missing. Hence skipped product SKU #$item->id", $this->logPath);
			$this->customHelper->writeCustomLog("Attribute set ID #$attid is missing. Hence skipped product SKU #$item->id", $this->logPath);
			// end ktpl0123 custom log
    	}
    }
        ...
        if ($product){
            ...
            foreach($multiple_values as $attribute_code => $attribute_values){
                ...
                if(!$attr_id){
                    ...
                    // start ktpl0123 custom log
					$this->customHelper->writeCustomLog('xmlimport : attribute '.$attribute_code . ' is not available in magento database.Hence skipping product having id ' . (string)$item->id, $this->logPath);
					// end ktpl0123 custom log
                }
                else{
                    $attr_type = $loadedattr->getFrontendInput();
                    ...
                    if($attr_type =='select' && count($attribute_values)>1){
                        ...
                        // start ktpl0123 custom log
						$this->customHelper->writeCustomLog('xmlimport : Attribute '. $attribute_code. ' can not have multiple values. Hence skipping product having id ' . (string)$item->id, $this->logPath);
						// end ktpl0123 custom log
                        $skipStatus = 1;
                        break;
                    }
    ...
    public function lookup($listType){
        ...
        $t1 = microtime(true);
        $time = $t1-$t0;  
         Mage::log("xmlimport : Found $count records","startup");
        // start ktpl0123 custom log
		$this->customHelper->writeCustomLog("xmlimport : Found $count records", $this->logPath);
		// end ktpl0123 custom log
    }
    ...
    protected function _initCategory($categoryId, $store = null){
        try{
            ...
            if (!$category->getId()) {
				// start ktpl0123 custom log
				$this->customHelper->writeCustomLog(Mage::helper('catalog')->__('xmlimport : Parent category $categoryId is not available.'), $this->logPath);
				$this->customHelper->sendLogEmailAndRemoveLog($this->logPath);
				// end ktpl0123 custom log
                Mage::throwException(Mage::helper('catalog')->__('xmlimport : Parent category "%s" is not available.', $categoryId));
            }
        }
        catch (Exception $e){
			// start ktpl0123 custom log
    ...
    public function associateProductToCategory( $productDetail,$catId){   //actual id of category
        ...
        if($newProduct){
            ...
            // start ktpl0123 custom log
			$this->customHelper->writeCustomLog('xmlimport : product does not exists :'.(string)$productDetail->id, $this->logPath);
			// end ktpl0123 custom log
            ...
            if($externall){
                if(count($externall) == 1){
                    ...
                    foreach($associate->product as $product){
                        Mage::log("xmlimport : Start Process for category # ".$first_key." and product # ".$product->id. "association");
                        // start ktpl0123 custom log
						$this->customHelper->writeCustomLog("xmlimport : Start Process for category # ".$first_key." and product # ".$product->id. "association", $this->logPath);
						// end ktpl0123 custom log
                        $this->associateProductToCategory($product,$first_key);
                        Mage::log("xmlimport : End Process for category # ".$first_key." and product # ".$product->id. "association");
                        // start ktpl0123 custom log
						$this->customHelper->writeCustomLog("xmlimport : End Process for category # ".$first_key." and product # ".$product->id. "association", $this->logPath);
						// end ktpl0123 custom log
                    }
                }
                else{
                	foreach($externall as $systemCatid=>$v){
                		foreach($associate->product as $product){
                		    Mage::log("xmlimport : Start Process for category # ".$systemCatid." and product # ".$product->id. "association");
                		    // start ktpl0123 custom log
							$this->customHelper->writeCustomLog("xmlimport : Start Process for category # ".$systemCatid." and product # ".$product->id. "association", $this->logPath);
							// end ktpl0123 custom log
                			$this->associateProductToCategory($product,$systemCatid);
                			Mage::log("xmlimport : End Process for category # ".$systemCatid." and product # ".$product->id. "association");
                			// start ktpl0123 custom log
							$this->customHelper->writeCustomLog("xmlimport : End Process for category # ".$systemCatid." and product # ".$product->id. "association", $this->logPath);
							// end ktpl0123 custom log
                		}
                	}               	               	
                }
                ...
                if(count($externall) == 0){
                    Mage::log('xmlimport : category id not found: '.$parent);
                    // start ktpl0123 custom log
					$this->customHelper->writeCustomLog('xmlimport : category id not found: '.$parent, $this->logPath);
					// end ktpl0123 custom log
                }
            }
        }
    ...
    public function associatePdtPdt($association){ 
        foreach($association as $associate){
            ...
            // start ktpl0123 custom log
			$this->customHelper->writeCustomLog("xmlimport : Start Process for product association # ".$associate->productIdFrom, $this->logPath);
			// end ktpl0123 custom log
            ...
            if ($mainProduct) {
				...
				// start ktpl0123 custom log
				$this->customHelper->writeCustomLog('xmlimport : product not found for association: # '.$associate->productIdFrom, $this->logPath);
				// end ktpl0123 custom log
            }
            Mage::log("xmlimport : End Process for product association # ".$associate->productIdFrom);
            // start ktpl0123 custom log
			$this->customHelper->writeCustomLog("xmlimport : End Process for product association # ".$associate->productIdFrom, $this->logPath);
			// end ktpl0123 custom log
        }
    }
    ...
    public function associatedProductsProducts(){ //Get product Association from XML
        ...
        if( $xmlData->productAssociations->association instanceof Varien_Simplexml_Element){
            ...
            Mage::log('xmlimport : Associatation block is empty');
            // start ktpl0123 custom log
			$this->customHelper->writeCustomLog('xmlimport : Associatation block is empty', $this->logPath);
			// end ktpl0123 custom log
        }
    }
                ...
                if($attr_configurable = $attr->getIsConfigurable()){
                    ...
                    if($attribute_type == 'select'){
                        ...
                        $result =  true;
                         ...
                         Mage::log('xmlimport : attribute '.$attribute_id. ' can not be updated to become superattribute');
                        // start ktpl0123 custom log
						$this->customHelper->writeCustomLog('xmlimport : attribute '.$attribute_id. ' can not be updated to become superattribute', $this->logPath);
						// end ktpl0123 custom log
                        $result =  false;
                    }
                }
            }else{
                Mage::log('xmlimport : attribute '.$attribute_id.' does not exists');
                // start ktpl0123 custom log
				$this->customHelper->writeCustomLog('xmlimport : attribute '.$attribute_id.' does not exists', $this->logPath);
				// end ktpl0123 custom log
                $result =  false;
            }
        }
     ...
     */            
				...
				// start ktpl0123 custom log
				$this->customHelper->writeCustomLog('Sorry, error occured while trying to save the attribute. Error: '.$e->getMessage(), $this->logPath);
				$this->customHelper->sendLogEmailAndRemoveLog($this->logPath);
				// end ktpl0123 custom log
    ...
    public function importAttribute($parsedAttribute){
        foreach($parsedAttribute as $attribute){
            Mage::log("xmlimport : Start Process for attribute # ".$attribute->id);
            // start ktpl0123 custom log
			$this->customHelper->writeCustomLog("xmlimport : Start Process for attribute # ".$attribute->id, $this->logPath);
			// end ktpl0123 custom log
            $this->createAttribute($attribute);
            Mage::log("xmlimport : End Process for attribute # ".$attribute->id);
            // start ktpl0123 custom log
			$this->customHelper->writeCustomLog("xmlimport : End Process for attribute # ".$attribute->id, $this->logPath);
			// end ktpl0123 custom log
        }
    ...
    public function importAttributeSet($parsedAttributeSet){
        ...
        foreach($parsedAttributeSet as $set){
			// start ktpl0123 custom log
			$this->customHelper->writeCustomLog("xmlimport : Start Process for attribute # ".$set->id, $this->logPath);
			// end ktpl0123 custom log
             Mage::log("xmlimport : Start Process for attribute # ".$set->id); 
            ...
            Mage::log("xmlimport : End Process for attribute # ".$set->id);
            // start ktpl0123 custom log
			$this->customHelper->writeCustomLog("xmlimport : End Process for attribute # ".$set->id, $this->logPath);
			// end ktpl0123 custom log
        }
    ...
    public function createAttributeSet($attribute_set_name, $external_id ) {
        ...
        if(isset($attributeSetId) && !empty($attributeSetId)) {
            ...
            if (!$modelSet->getId()) {
				// start ktpl0123 custom log
				$this->customHelper->writeCustomLog(Mage::helper('catalog')->__('This attribute set no longer exists.'), $this->logPath);
				// end ktpl0123 custom log
        ...
        try{
			...
			// start ktpl0123 custom log
			$this->customHelper->writeCustomLog('Attribute set name '.$attribute_set_name.' with id '.$external_id.' already exists in Magento system with same name', $this->logPath);
			$this->customHelper->sendLogEmailAndRemoveLog($this->logPath);
			// end ktpl0123 custom log
    ...
    public function removeAttributeGroup($attribute_group_name ,$attribute_group_id, $attributeSetId){
        ...
        if($attributeGroupId){
            ...
            // start ktpl0123 custom log
			$this->customHelper->writeCustomLog('Attribute Group is not available to be removed', $this->logPath);
			// end ktpl0123 custom log
        }
    }
       ...
       if(isset($attributeGroupId) && !empty($attributeGroupId)) {
            ...
            if( $model->itemExists() ) {
                ...
                try {
					...
					// start ktpl0123 custom log
					$this->customHelper->writeCustomLog(Mage::helper('catalog')->__('An error occurred while saving this group.'), $this->logPath);
					$this->customHelper->sendLogEmailAndRemoveLog($this->logPath);
					// end ktpl0123 custom log
