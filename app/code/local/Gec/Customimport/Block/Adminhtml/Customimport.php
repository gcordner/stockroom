	...
	*/
    ...
    public function createProduct(&$item, $asid)
    {
    	...
    	if (!empty($attributeSetModel))
    	{
			...
			elseif ($manageItem=='N')
	    	{
				$product->setStockData(array('use_config_backorders' => 0,
											'manage_stock' => 0));
			}
    ...
    public function getItemIds($item,$Currfilepath=null,$Errfilepath=null)
    {
		...
		else 
		{
			...
			$attributeSetId = $attributeSetModel->getCollection()
		...
		if($attributeSetId)
		{
        	...
        	if($sku!= $curitemids)
	        {
	            ...
	            {
	                $this->_curitemids=array("pid"=>null,"sku"=>$sku,"asid"=>isset($attributeSetId)?$attributeSetId:null);
	            }
