...
class Bronto_Order_Model_Observer
{
    ...
    public function processOrdersForStore($storeId, $limit)
    {
        ...
        /** @var Bronto_Api_Write $addOrUpdateOrders */
    ...
    /**
     * Process orders by sending data to the Order Service
     * 
     ...
     * @throws Exception
     */
    public function processOrdersForStoreOs(\Mage_Core_Model_Store $store, $limit)
    {
        $result = array('total' => 0, 'success' => 0, 'error' => 0);
        ...
        $this->_helper->writeDebug("Starting Order Import process for store: {$store->getName()} ({$store->getId()})");

        $moduleEnabled = $store->getConfig(Bronto_Order_Helper_Data::XML_PATH_ENABLED);
        if (!$moduleEnabled) {
            $this->_helper->writeVerboseDebug('  Module disabled for this store. Skipping...');
            return $result;
        }

        // Retrieve REST Client ID and Client Secret
        $clientId = $store->getConfig(Bronto_Common_Helper_Api::XML_PATH_REST_CLIENT_ID);
        $clientSecret = $store->getConfig(Bronto_Common_Helper_Api::XML_PATH_REST_CLIENT_SECRET);
        $restClient = null;
        if (!$clientId || !$clientSecret) {
            $this->_helper->writeError("  Client ID and/or Secret was not loaded. Check to ensure these values are set.");
            return $result;
        }
        $restClient = \Bronto_Order_Api_Rest_ClientFactory::getInstance()->getByClientId($clientId);
        if (!$restClient) {
            $restClient = \Bronto_Order_Api_Rest_ClientFactory::getInstance()->create($clientId, $clientSecret);
        }

        // Figure out what needs to be imported and what needs to be deleted
        /* @var $productHelper Bronto_Common_Helper_Product */
        $productHelper   = Mage::helper('bronto_common/product');
        $allStates       = Mage::getModel('bronto_order/system_config_source_order_state')->toArray();
        $importStateMap  = $this->_createStateMap($allStates, $this->_helper->getImportStates('store', $store->getId()));
        ...
        // Pull the orders queued for import
        /** @var Bronto_Order_Model_Queue[] $orderQueueItems */
        $orderQueueItems = Mage::getModel('bronto_order/queue')
            ->getCollection()
            ->addBrontoNotImportedFilter()
            ->addBrontoNotSuppressedFilter()
            ->addBrontoHasOrderFilter()
        ...
        if (empty($orderQueueItems)) {
            $this->_helper->writeDebug('  No Orders to process. Skipping...');
            return $result;
        }

        // Process the orders queued for import
        foreach ($orderQueueItems as $orderQueueItem) {
            /** @var Mage_Sales_Model_Order $order */
            $order = \Mage::getModel('sales/order')->load($orderQueueItem->getOrderId());

            if (!$order->getId()) {
                $this->_skipOrder($order, $orderQueueItem, "Invalid order");
                $result['success']++;
                continue;
            }
            ...
            if (!$orderDto->getCustomerOrderId()) {
                ...
                $success = $this->importOrder($restClient, $orderQueueItem, $orderDto);
                $result[$success ? 'success' : 'error']++;
            } else if (array_key_exists($order->getState(), $deleteStateMap)) {
                $success = $this->deleteOrder($restClient, $orderQueueItem, $orderDto);
                $result[$success ? 'success' : 'error']++;
            } else {
                $this->_skipOrder($order, $orderQueueItem, "Invalid state {$allStates[$order->getState()]}");
                $result['success']++;
            }
        }
    ...
    /**
     * @param Bronto_Order_Api_Rest_Client $restClient
     * @param Bronto_Order_Model_Queue $orderQueueItem
     * @param Bronto_Order_Api_Rest_Dto $orderDto
     * @return bool
     */
    protected function importOrder(
        Bronto_Order_Api_Rest_Client $restClient,
        Bronto_Order_Model_Queue $orderQueueItem,
        Bronto_Order_Api_Rest_Dto $orderDto
    ) {
        try {
            $processDate = Mage::getSingleton('core/date')->gmtDate();
    ...
    protected function deleteOrder(
        ...
        try {
            $processDate = Mage::getSingleton('core/date')->gmtDate();
            $restClient->deleteOrder($orderDto->getCustomerOrderId());
            $orderQueueItem->setBrontoImported($processDate)->save();
            return true;
        } catch (\Bronto_Api_Exception $bae) {
            $this->_helper->writeInfo(
                ($bae->getMessage()
                    . "\n " . $bae->getPrevious()->getMessage()
                    . "\n " . $restClient->getHttpClient()->getLastRequest())
            );
            $orderQueueItem->setBrontoImported(null)->setBrontoSuppressed("{$bae->getMessage()}")->save();
            return false;
        }
    ...
    public function processOrders($brontoCron = false)
    {
        ...
        if (Mage::helper('bronto_order')->canUseMageCron() || $brontoCron) {
            ...
            foreach ($stores as $_store) {
                ...
                $useOrderService = $_store->getConfig(\Bronto_Order_Helper_Data::XML_PATH_ORDER_REST_ENABLED);
                $storeResult = $useOrderService
                    ? $this->processOrdersForStoreOs($_store, $limit)
                    : $this->processOrdersForStore($_store, $limit);
