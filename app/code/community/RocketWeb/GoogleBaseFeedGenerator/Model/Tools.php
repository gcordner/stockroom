...
class RocketWeb_GoogleBaseFeedGenerator_Model_Tools extends Varien_Object
{
    ...
    public function loadEntityType($type)
    {
        if (is_array($type)) {
            ...
            if (!Mage::registry('googlebasefeedgenerator/entity_type/' . $type)) {
    ...
    public function getConfigurableAttribute($attribute_code, $product_id = null)
    {
        ...
        $row = array();
        $websiteId = Mage::helper('googlebasefeedgenerator')->getGenerator($this->getFeed())->getData('website_id');
    ...
    {
        $skip = false;
        $generator = Mage::helper('googlebasefeedgenerator')->getGenerator($this->getFeed());
        ...
        if (!empty($is_assoc_configurable)
            ...
            && $this->getFeed()->getConfig('auto_skip')
        ) {
    ...
    {
        return $this->getFeed()->getConfig('options_product_options_mode') == RocketWeb_GoogleBaseFeedGenerator_Model_Source_Product_Option::MULTIPLE_ROWS;
    }
}
