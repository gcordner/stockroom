    ...
    extends Mage_Adminhtml_Block_System_Config_Form_Field
{
     ...
     * i.e. category_id, value (taxonomy line), disabled status and order (not needed)
     * TODO: remove order
     *
    ...
    public function getFieldValues($categoryId = 0)
    {
        ...
        if ($categoryId > 0) {
            if (isset($this->_fieldValues[$categoryId])) {
                if (!array_key_exists('disabled', $this->_fieldValues[$categoryId])) {
                    $this->_fieldValues[$categoryId]['disabled'] = false;
                }
            ...
            return array(
                'category' => $categoryId,
                'value'    => '',
                'disabled' => $this->getFeed()->getId() ? true : false //If this is a new feed, then enable all categories by default
            );
        }
