...
class Bronto_Reminder_Block_Adminhtml_Reminder_Edit_Tab_Bronto extends Mage_Adminhtml_Block_Widget_Form
{
    ...
    protected function _prepareForm()
    {
        ...
        foreach (Mage::app()->getWebsites() as $website) {
            ...
            $fieldset->addField(
                "website_message_{$website->getId()}", 'note', array(
            ...
            foreach ($website->getGroups() as $gkey => $group) {
                ...
                $fieldset->addField(
                    "group_message_{$group->getId()}", 'note', array(
                    'label'               => $group->getName(),
                ...
                foreach ($stores as $key => $store) {
                    if (Mage::helper('bronto_reminder')->isEnabled('store', $store->getId())) {
                        ...
                        $fieldset->addField(
                            "store_message_{$store->getId()}", 'select', array(
                        ...
                        $fieldset->addField(
                            "store_message_sendflags_{$store->getId()}", 'select', array(
                        ...
                        $fieldset->addField(
                            "store_message_sendtype_{$store->getId()}", 'radios', array(
