...
class RocketWeb_GoogleBaseFeedGenerator_Model_Generator extends Varien_Object
{
    ...
    public function processProductCallback($args)
    {
        ...
        if ($this->isMemoryOverLimit()) {
            // Automatically swicth to batch mode
            if (!$this->inBatch()) {
                $this->setBatchMode(1);
                if ($this->getBatch()->aquireLock()) {
                    $this->getFeed()->getSchedule()
                        ->addData(array('batch_mode' => 1, 'batch_limit' => $this->_current_iter))
                        ->save();
                    $this->log('Memory Limit reached. Automatic swicth to batch mode.');
                } else {
                    throw new Exception('Memory Limit reached');
                }
            }
            // We are terminating early
            $this->getBatch()->updateLockOffset($this->_current_iter);
            $this->releaseLock();
            // Exit the iterator but don't set the feed as failed
            throw new RocketWeb_GoogleBaseFeedGenerator_Model_Exception('EARLY END / ALL MEMORY USED: ' . $this->formatMemory(memory_get_usage(true)));
        }
    ...
    public function logProgress()
    {
        ...
        if ($time->get(Zend_Date::TIMESTAMP) - $this->getProgressTiming() > 15
            ...
            || ($this->inBatch() && ($this->_current_iter % $this->getBatch()->getLimit() == 0 || $this->isMemoryOverLimit())))
        {
    ...
    protected function _getCollection()
    {
        ...
        foreach ($categoryMap as $category) {
            if (isset($category['category']) && isset($category['disabled']) && (bool)$category['disabled'] === false) {
                $includeCategories[] = (int)$category['category'];
            }
        }
        $collection = $this->_addCategoriesToFilter($collection, $includeCategories, (bool)$includeAllProducts);
    ...
    {
        if ($includeAllProducts === false && count($categoryIds) == 0) {
            Mage::throwException('No categories were selected! Please check Categories Map settings.');
        }

        $where = array();
        if ($includeAllProducts === true) {
            $where[] = 'cat_product.category_id IS NULL';
        ...
        if (count($categoryIds) > 0) {
            $where[] = $collection->getConnection()->quoteInto('cat_index.category_id IN (' . implode(',', $categoryIds) . ')', "");
            $conditions = array(
                'cat_index.product_id=e.entity_id',
                $collection->getConnection()->quoteInto('cat_index.store_id=?', $this->getData('store_id')),
            );
     ...
     * @return bool
     */
    public function isMemoryOverLimit()
    {
