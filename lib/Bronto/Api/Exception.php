...
class Bronto_Api_Exception extends RuntimeException
{
    ...
    public function __construct($message = "", $code = 0, Exception $previous = null, $tries = 0, $request = null)
    {
        ...
        if (empty($code)) {
            ...
            } else if (strpos($message, 'Unable to read response, or response is empty') !== false) {
                $code = self::EMPTY_RESPONSE;
            }
        }
    ...
    /**
     * Is Access Token expired or is Session Token invalid?
     *
     * @return bool
     */
    public function isTokenInvalid()
    {
        return $this->getCode() == self::INVALID_TOKEN
        || $this->isInvalidSession()
        || $this->getCode() == self::FORBIDDEN;
    }
