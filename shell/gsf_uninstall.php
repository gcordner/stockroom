...
class GoogleBaseFeedGenerator_Shell_Uninstall extends Mage_Shell_Abstract
{
    ...
    public function deleteTables()
    {
        ...
        $con->query("DROP TABLE IF EXISTS `{$res->getTableName('rw_gfeed_feed_config')}`");
        $con->query("DROP TABLE IF EXISTS `{$res->getTableName('rw_gfeed_feed_schedule')}`");
        $con->query("DROP TABLE IF EXISTS `{$res->getTableName('rw_gfeed_queue')}`");
