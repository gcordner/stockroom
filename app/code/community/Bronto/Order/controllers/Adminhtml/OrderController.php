...
class Bronto_Order_Adminhtml_OrderController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function runAction()
    {
        ...
        /** @var \Bronto_Order_Model_Observer $model */
        $model  = Mage::getModel('bronto_order/observer');
        
        /** @var \Bronto_Order_Helper_Data $helper */
        $helper = Mage::helper('bronto_order');
        
        $limit  = $helper->getLimit();

        try {
            if ($stores = $helper->getStores()) {
                foreach ($stores as $store) {
                    if ($limit <= 0) {
                        continue;
                    }

                    $useOrderService = $store->getConfig(\Bronto_Order_Helper_Data::XML_PATH_ORDER_REST_ENABLED);
                    $storeResult = $useOrderService
                        ? $model->processOrdersForStoreOs($store, $limit)
                        : $model->processOrdersForStore($store->getId(), $limit);
