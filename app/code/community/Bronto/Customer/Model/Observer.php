    ...
    protected function _processDynamicAttributes($attributesFields = array(), $section, $group)
    {
        ...
        foreach ($attributesFields as $fieldId => $field) {
            ...
            if (preg_match('/_dynamic_new/', $fieldId) || preg_match('/_new/', $fieldId)) {
                ...
                try {
                    /* @var $fieldObject Bronto_Api_Operation */
                    $fieldObject        = $api->transferField();
                    ...
                    $errMessage = "Unable to save new field: {$value}";
                    Mage::getSingleton('core/session')->addError("{$errMessage}. See Bronto logs for additional information.");
                    Mage::helper('bronto_customer')->writeError("{$errMessage}: {$e->getMessage()}");
                }
            } // Save Dynamic Fields
